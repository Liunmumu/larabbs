    ...
    @foreach ($topics as $topic)
      <li class="d-flex">
        ...
        <div class="flex-grow-1 ms-2">
          ...
          <div class="mt-0 mb-1">
            <a href="{{ $topic->link() }}" title="{{ $topic->title }}" title="{{ $topic->title }}">
              {{ $topic->title }}
            </a>
            <a class="float-end" href="{{ $topic->link() }}" title="{{ $topic->title }}">
          ...
          <small class="media-body meta text-secondary">
            ...
            <span class="timeago" title="Last active in{{ $topic->updated_at }}">{{ $topic->updated_at->diffForHumans() }}</span>
          </small>
